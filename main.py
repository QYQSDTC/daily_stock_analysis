...
示例:
  ...
  python main.py --no-dragon-hunt   # 跳过擒龙策略（完整分析模式下）
  python main.py --dragon-hunt      # 仅运行短线擒龙策略分析（含 AI 深度分析）
  python main.py --dragon-hunt --dragon-no-ai  # 擒龙策略（禁用 AI）
  python main.py --dragon-hunt --dragon-top-n 3  # 只分析前3只龙头
        """,
    )
    ...
    parser.add_argument("--dragon-hunt", action="store_true", help="仅运行短线擒龙策略分析")

    parser.add_argument("--no-dragon-hunt", action="store_true", help="跳过短线擒龙策略分析")

    parser.add_argument("--dragon-no-ai", action="store_true", help="擒龙策略禁用 AI 分析")

    parser.add_argument("--dragon-top-n", type=int, default=5, help="擒龙策略 AI 分析前 N 只龙头股（默认5）")
...
def run_dragon_hunt_analysis(notifier: NotificationService, enable_ai: bool = True, top_n: int = 5) -> Optional[str]:
    """
    执行短线擒龙策略分析
    ...
    Returns:
        擒龙报告文本
    """
    logger.info(f"开始执行短线擒龙策略分析 (AI: {'启用' if enable_ai else '禁用'}, 龙头分析数: {top_n})...")

    try:
        from dragon_hunter import run_dragon_hunt

        report = run_dragon_hunt(enable_ai=enable_ai, top_n_dragons=top_n)

        if report:
            # 保存报告到文件
            date_str = datetime.now().strftime("%Y%m%d")
            report_filename = f"dragon_hunt_{date_str}.md"
            filepath = notifier.save_report_to_file(report, report_filename)
            logger.info(f"擒龙报告已保存: {filepath}")

            # 推送通知
            if notifier.is_available():
                success = notifier.send(report)
                if success:
                    logger.info("擒龙报告推送成功")
                else:
                    logger.warning("擒龙报告推送失败")
...
def run_full_analysis(config: Config, args: argparse.Namespace, stock_codes: Optional[List[str]] = None):
    ...
    try:
        ...
        # 3. 运行短线擒龙策略（如果启用）
        dragon_report = ""
        if config.dragon_hunt_enabled and not getattr(args, "no_dragon_hunt", False):
            dragon_result = run_dragon_hunt_analysis(
                notifier=pipeline.notifier,
                enable_ai=config.dragon_hunt_ai_enabled,
                top_n=config.dragon_hunt_top_n,
            )
            if dragon_result:
                dragon_report = dragon_result
        ...
        try:
            ...
            if feishu_doc.is_configured() and (results or market_report or dragon_report):
...
def main() -> int:
    ...
    try:
        # 模式0: 短线擒龙策略
        if args.dragon_hunt:
            enable_ai = not args.dragon_no_ai
            top_n = args.dragon_top_n
            logger.info(f"模式: 短线擒龙策略 (AI: {'启用' if enable_ai else '禁用'}, 龙头分析数: {top_n})")
            from dragon_hunter import run_dragon_hunt
            ...
            try:
                ...
                # 保存报告
                date_str = datetime.now().strftime("%Y%m%d")
                filepath = notifier.save_report_to_file(report, f"dragon_hunt_{date_str}.md")
                logger.info(f"擒龙报告已保存: {filepath}")
